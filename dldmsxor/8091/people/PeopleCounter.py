...
cnt_down = 0
count=0
cap = cv.VideoCapture('Peopletest77.avi')
#cap = cv.VideoCapture(1)
#cap=cv.VideoCapture('http://113.198.137.189:8091/?action=stream')
#cap=cv.VideoCapture('http://119.196.44.122:'+camera+'/?action=stream')
...
try:
	cur=conn.cursor()
	sql="select username from Camera where cid="+camera
	cur.execute(sql)
	name=cur.fetchone()
	
	query = "INSERT INTO people_count VALUES(%s,%s,%s,%s,%s,%s)"
	cur.execute(query,(0,name,camera,cnt_up,cnt_down,date))
	conn.commit()
finally:
	cur.close()
	conn.close()
